...
Im Rahmen dieses Tutorials zur Itemanalyse verwenden wir echte Daten aus einem im vergangenen Semester konstruierten Fragebogen zu **Umweltbewusstsein**, welches die Einstellung von Personen gegenüber dem Umweltschutz widerspiegelt. Spezifisch wurde dieses Konstrukt auf der Verhaltensebene operationalisiert, wobei mehr oder weniger umweltschützende Verhaltensweisen als Indikator für das Ausmaß des umweltbewussten Handelns betrachtet werden. Neben den 29 Items der Rohfassung des Fragebogens enthält der Datensatz zusätzlich demographische Informationen über das Alter, Geschlecht, Wohnsituation sowie Bildungsgrad der 104 Proband:innen.
...
Der Datensatz ist in diesem Fall als `rds`-Datei gespeichert und kann mittels base-R (`readRDS()`) oder des readr-Package (`readr::read_rds()`) eingelesen werden.

::: callout-note
## Information zum Einlesen von Daten

Wenn ihr euren Datensatz aus SoSci-Survey herunterladet, wird dieser in einem anderen Format repräsentiert sein (typischerweise entweder .csv, oder .xlsx). Um csv-Dateien (mit entweder Komma, Semikolon oder Tab Separator) einzulesen, können entweder base-R Funktionen (`read.csv()`, `read.csv2()` oder `read.delim()`) oder z.B. Funktionen des *readr-*Packages aus der Familie des [*tidyverse*](https://www.tidyverse.org/) genutzt werden (`read_csv()`, `read_csv2()` und `read_tsv()`). Um hingegen Excel-Dateien lesen zu können, kann das *readxl*-Package genutzt werden (`read_excel()`)
:::

Wir laden sowohl den gesamten Datensatz und speichern zudem ein subset des Datensatzes, welches nur die Items zu Umweltbewusstsein enthält. Hilfreiche Funktionen, um sich einen ersten Überblick zu schaffen, sind zum Beispiel `skim()` oder `str()`.
